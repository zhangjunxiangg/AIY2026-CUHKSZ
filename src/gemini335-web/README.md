...
Web 服务端点：`/`（监视页，滤波深度）、`/raw`（未滤波对比页）、`/locate`（**机器人定位可视化页**）、`/color.mjpeg`、`/depth.mjpeg`（滤波）、`/depth_raw.mjpeg`（原始）、`/locate.mjpeg`（定位叠加流）、`/snapshot/*.jpg`、`/health`（JSON，含滤波器状态）、`/locate/status`（定位状态 JSON）、`/locate/debug`（候选/簇分解 JSON，含 simple 兜底埋点）、`/locate/debugimg`（主链掩码可视化图）、`/locate/debugimg2`（兜底掩码可视化图）、`POST /locate/recalibrate`（重新标定地面）。
...
### 简单兜底检测器（2026-08-06，`_simple_robot_detect`）

主链（DBSCAN）找不到机器人时自动接管，输出青色框 `robot(simple)`；原管线一行未动，主链有结果时永远优先。实测：主链漏检时兜底成功接管，连续 90 秒输出位置抖动 ≤4mm。

算法（比主链简单得多，全程单帧、无聚类）：

1. **场地区域** = 地面平面内点 − 品红地毯 的最大连通域的**凸包**（几何+色谱，无亮度门限——暗光下固定灰度门限会整张崩掉；凸包天然覆盖机器人遮挡洞/线缆/阴影，洞填法在这里会因遮挡沟道连通场界而失效）
2. **机器人** = 场地内最大"深色中性"连通域：灰度 < min(140, 0.72×垫子灰度中位数)（相对阈值自适应灯光）且低饱和且非蓝；开运算去碎点 + 45px 闭运算把桅杆/底盘/机械臂/影子合并成一团
3. **锥桶排除**：暗蓝底座（blue_dom>30 且灰度<130——机器人屏幕是亮蓝，不会误判为锥桶）生成向上扩展的排除框
4. 位置 = blob 底带反投影∩地面平面，与主链同一套场地坐标系和围栏过滤

调试端点：`/locate/debug` 的 `simple` 段含每级计数/组件列表/选中框（掩码数组不序列化）；`/locate/debugimg2` 直接出掩码叠加图（绿=场地凸包，红=深色 blob，蓝=锥桶种子）。

**为什么兜底不能用深度做（实测结论）**：这颗 Gemini 335 的深度管线带孔洞填充，机器人在深度图里被"抹"进地面——高度残差和零值洞两个信号都捞不到它（垫子上 h_max 只有 ~90mm）。深度占用检测对浅色物体（托盘/锥桶）有效，对黑色机器人无效。

**仍存的边界**：暗光下机器人银灰机身与垫子亮度接近（机身 p50≈150 vs 垫子 p50≈172），相对阈值只能抓到桅杆/轮子/关节等深色件，靠 45px 闭运算合并；灯光正常时整机成团。相机被碰动后必须点"重新标定地面"（内点掩码和平面都依赖标定时刻的几何）。
