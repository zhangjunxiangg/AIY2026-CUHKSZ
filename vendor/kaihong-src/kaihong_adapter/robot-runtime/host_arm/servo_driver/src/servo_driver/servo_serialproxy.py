...
class SerialProxy:
    def __init__(self,
        ...
        self.fallback_ids = set(fallback_ids)
        self.running = False
        self.servo_io = None
    ...
    def connect(self):
        ...
        if self.update_rate > 0:
            Thread(target=self.__update_servo_states, daemon=True).start()
    ...
    def __update_servo_states(self):
        ...
        while not rospy.is_shutdown() and self.running:
            ...
            for servo_id in self.servos:
                try:
                    state = self.servo_io.get_feedback(
                        servo_id,
                        self.fake_read,
                        servo_id in self.fallback_ids,
                    )
                    if state:
