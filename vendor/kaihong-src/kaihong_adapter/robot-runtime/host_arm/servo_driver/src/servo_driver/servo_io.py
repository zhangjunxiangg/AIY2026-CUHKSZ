...
class ServoIO:
    def __init__(self):
        self.ser = None
        # Keep a transient bus miss from stalling the feedback publisher for
        # several seconds per joint; failed reads are handled by the caller.
        self.timeout = 2
    ...
    def ping(self, servo_id):
        ...
        msg.get_id = 1
        for i in range(0, 20):
            res = rospy.ServiceProxy('/ros_robot_controller/bus_servo/get_state', GetBusServoState)([msg]).state
            if res and servo_id in res[0].present_id:
                return True
        return False
    ...
    def get_voltage(self, servo_id):
        ...
        while True:
            count += 1
            res = rospy.ServiceProxy('/ros_robot_controller/bus_servo/get_state', GetBusServoState)([msg]).state
            if res and res[0].voltage:
                return res[0].voltage[0]
            if count > self.timeout:
                return None

    def get_feedback(self, servo_id, fake_read=False, fallback_to_goal=False):
        position = self.get_position(servo_id, fake_read)
        if position is None and fallback_to_goal:
            # Some vendor servos accept commands and report voltage/temperature
            # but return 0xffff for position. Keep that joint usable in the
            # kinematics chain with an explicitly declared open-loop estimate.
            position = self.servos[servo_id].goal
            rospy.logwarn_throttle(
                30.0,
                "servo %d position feedback unavailable; using commanded position estimate",
                servo_id,
            )
        if position is not None:
            ...
            if not servo.initialized:
                # Align the software state with the first real read. Starting the
                # controller manager must never imply a homing movement.
                servo.goal = position
                servo.estimated_pos = position
    ...
    def get_servo_id(self, servo_id=None):
        ...
        while True:
            count += 1
            res = rospy.ServiceProxy('/ros_robot_controller/bus_servo/get_state', GetBusServoState)([msg]).state
            if res and res[0].present_id:
                return res[0].present_id[0]
            if count > self.timeout:
                return None
