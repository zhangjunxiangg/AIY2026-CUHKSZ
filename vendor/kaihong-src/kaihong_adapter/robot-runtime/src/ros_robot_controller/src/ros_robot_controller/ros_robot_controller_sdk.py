...
class Board:
    ...
    def __init__(self, device=None, baudrate=None, timeout=1, reconnect_interval=1.0):
        device = device or os.environ.get("RRC_DEVICE", "/dev/ttyCH343USB0")
        baudrate = int(baudrate or os.environ.get("RRC_BAUDRATE", "1000000"))
        self.device = device
        self.baudrate = baudrate
        self.timeout = timeout
        # A missing bus-servo reply must not block every ROS callback forever.
        # The controller normally answers within a few tens of milliseconds.
        self.bus_servo_response_timeout = 0.5
    ...
    def _open_port(self):
        port = serial.Serial(None, self.baudrate, timeout=self.timeout)
        try:
            port.rts = False
            port.dtr = False
            port.setPort(self.device)
            port.open()
        except Exception:
            try:
                port.close()
    ...
    def get_connection_generation(self):
    ...
    def buf_write(self, func, data):
        ...
        with self.write_lock:
            if not self.connected:
                raise serial.SerialException("RRC serial port is disconnected")
            try:
    ...
    def set_motor_type(self, motor_type=0x02):
        """Configure the RRC Lite motor model; this does not command motion."""
        self.buf_write(
            PacketFunction.PACKET_FUNC_MOTOR,
            struct.pack("<BB", 0x05, int(motor_type)),
        )

    def set_battery_level(self, battery_level=0x1AF4):
        """Configure the low-battery threshold in millivolts."""
        self.buf_write(
            PacketFunction.PACKET_FUNC_SYS,
            struct.pack("<BH", 0x01, int(battery_level)),
        )
    ...
    def bus_servo_enable_torque(self, servo_id, enable):
        # The 4.1 STM32 bus-servo firmware uses 0x0C for load/hold and 0x0B
        # for unload.  The vendor Python mapping shipped with this image was
        # reversed, which made an "enable" request release the arm joints.
        if enable:
    ...
    def bus_servo_read_and_unpack(self, servo_id, cmd, unpack):
        with self.servo_read_lock:
            # Discard a delayed reply from a previous request before issuing
            # the next command; the bus queue is intentionally single-entry.
            while True:
                try:
            ...
            deadline = time.monotonic() + self.bus_servo_response_timeout
            while True:
                remaining = deadline - time.monotonic()
                if remaining <= 0:
                    return None
                try:
                    data = self.bus_servo_queue.get(timeout=remaining)
                except queue.Empty:
                    return None
                response_id, response_cmd, success, *info = struct.unpack(unpack, data)
                if response_id != servo_id or response_cmd != cmd:
                    continue
                if success == 0:
                    return info
                return None
        ...
        while self.running:
            if self.enable_recv:
                if not self.connected:
                    ...
                    continue
                try:
                    # Read the currently buffered batch instead of one byte
                    # per Python call. The STM32 streams telemetry at high
                    # rate and byte-at-a-time reads waste most of one core.
                    available = int(getattr(self.port, "in_waiting", 0) or 0)
                    recv_data = self.port.read(min(max(available, 1), 4096))
                except (OSError, serial.SerialException):
                    self.connected = False
                    continue
                if recv_data:
                    ...
                    # On M-Robots OS 4.1 the CH343 serial implementation can
                    # return an empty buffer immediately even with a timeout.
                    # Avoid a full-core busy loop without delaying real data.
                    time.sleep(0.001)
            else:
                time.sleep(0.01)
